...
def update_holdings(current_words, numbers):
        ...
        try:
            if word in holdings:
                ...
                if num <= prev_num - risk_percentage or num >= prev_num + 1.0:
                    trigger(1763559069, f"SELL {word} at {num}%")
    ...
    for word in holdings:
        if word not in checked_stocks:
            trigger(1763559069, f"MISSING STOCK -> SELL {word}")
...
#     current_datetime = datetime.datetime.now()
#     formatted_datetime = current_datetime.strftime("%Y-%m-%d %H:%M:%S")
#     trigger(1763559069, formatted_datetime)
#     print("Done")
#     time.sleep(10)
        ...
        if new_words:
            for word in new_words:
                try:
                    ...
                    trigger(1763559069, f"BUY {word} at {numbers[index]}%")
